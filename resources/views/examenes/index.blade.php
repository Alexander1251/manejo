                            ...
                            <table id='Tabla' style="width:100%" class="table table-striped table-hover">
                                <thead class="thead">
                                    <tr>
                                        ...
                                        <th>Total de respuestas correctas</th>
                                        <th>Opciones</th>
                                ...
                                <tbody>
                                    ...
                                    @foreach ($examenes as $examen)
                                        <tr>
                                            ...
                                            <td>{{ $examen->expediente->cliente->nombres }}</td>
                                            <td>{{ $examen->expediente->cliente->apellidos }}</td>
                                            <td>{{ $examen->fecha_examen }}</td>
                                            <td>{{ $examen->calificacion }}</td>
                                            <td>{{ $examen->tiempo }}</td>
                                            <td>{{ $examen->total_preguntas }}</td>
                                            <td>{{ $examen->total_respuestas_correctas }}</td>
                                            <td>

                                                <form action="{{ route('examenes.destroy', $examen->id) }}" method="POST">
                                                    <a class="btn btn-sm btn-primary "
                                                        href="{{ route('examenes.show', $examen->id) }}"><i
                                                            class="fa fa-fw fa-eye"></i></a>

                                                    <a class="btn btn-sm btn-dark" target="_blank"
                                                        href="{{ route('examenes.detallePDF', $examen->id) }}"><i
